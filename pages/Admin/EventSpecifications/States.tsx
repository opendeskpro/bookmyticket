...
import { Plus, Search, Edit2, Trash2, X, Check, AlertCircle, Map } from 'lucide-react';
...
const States: React.FC = () => {
    ...
    const fetchCountries = async () => {
        const { data } = await supabase.from('countries').select('id, name').eq('status', 'active').order('name');
        setCountries(data || []);
    };
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to delete this state? This might affect associated cities.')) return;
        try {
            const { error } = await supabase
                .from('states')
                .delete()
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className={`text-sm mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-slate-500'}`}>
                        Manage states/provinces for your event locations.
                    </p>
                </div>
                <button
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-200/10">
                            {isLoading ? (
                                ...
                                <tr><td colSpan={5} className="text-center py-8 text-gray-500">No states found.</td></tr>
                            ) : (
                                states.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())).map((state) => (
                                    <tr key={state.id} className={`group transition-colors ${theme === 'dark' ? 'hover:bg-[#131922]/50 border-gray-700' : 'hover:bg-slate-50/80 border-slate-100'}`}>
                                        <td className="px-6 py-4 font-mono text-xs opacity-60">#{state.serial_number}</td>
                                        <td className="px-6 py-4 font-medium flex items-center gap-3">
                                            <Map size={16} className="text-purple-500 opacity-50" />
                                            {state.name}
                                        </td>
                                        <td className="px-6 py-4 text-xs font-bold uppercase tracking-wider opacity-70">
                                            {state.countries?.name || 'Unknown'}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${state.status === 'active'
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className={`w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 ${theme === 'dark' ? 'bg-[#1e2736] border border-white/10' : 'bg-white'}`}>
                        <div className={`px-6 py-4 border-b flex items-center justify-between ${theme === 'dark' ? 'border-gray-700 bg-[#131922]' : 'border-slate-100 bg-slate-50'}`}>
                            <h3 className="font-bold text-lg">{editingId ? 'Edit State' : 'Add New State'}</h3>
                            <button
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold uppercase tracking-wider opacity-70">State Name</label>
                                <input
                                    type="text"
                                    required
                                    value={formData.name}
